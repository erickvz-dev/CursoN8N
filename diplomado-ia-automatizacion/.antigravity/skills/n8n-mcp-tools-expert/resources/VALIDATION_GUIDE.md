...
The `mcp_n8n-mcp_validate_node` tool provides all validation capabilities with different modes.
...
## mcp_n8n-mcp_n8n_validate_workflow (STRUCTURE VALIDATION)
...
1. Configure node
   ↓
2. mcp_n8n-mcp_validate_node (23s thinking about errors)
   ↓
3. Fix errors
   ↓
4. mcp_n8n-mcp_validate_node again (58s fixing)
   ↓
...
const result1 = mcp_n8n-mcp_validate_node({
  nodeType: "nodes-base.slack",
  config,
  profile: "runtime"
});
...
const result2 = mcp_n8n-mcp_validate_node({
  nodeType: "nodes-base.slack",
  config,
  profile: "runtime"
});
...
- `mcp_n8n-mcp_n8n_autofix_workflow({id})` - preview/apply fixes

---

## mcp_n8n-mcp_n8n_autofix_workflow (AUTO-FIX TOOL)
...
**"Invalid value"**
→ Check allowed values in mcp_n8n-mcp_get_node output
...
- Use `mcp_n8n-mcp_n8n_autofix_workflow` for bulk fixes
...
6. Use `mcp_n8n-mcp_n8n_autofix_workflow` for automatic fixes

**Tool Selection**:
- **mcp_n8n-mcp_validate_node({mode: "minimal"})**: Quick required fields check
- **mcp_n8n-mcp_validate_node({profile: "runtime"})**: Full config validation (**use this!**)
- **mcp_n8n-mcp_n8n_validate_workflow**: Complete workflow check
- **mcp_n8n-mcp_n8n_validate_workflow({id})**: Validate existing workflow
- **mcp_n8n-mcp_n8n_autofix_workflow({id})**: Auto-fix common issues
