...
## mcp_n8n-mcp_search_nodes (START HERE!)
...
The `mcp_n8n-mcp_get_node` tool provides all node information with different detail levels and modes.
...
**Most common pattern**: search → mcp_n8n-mcp_get_node (18s average)
...
|-----------|-------------|-------|------|
| `mcp_n8n-mcp_search_nodes` | Find by keyword | <20ms | Small |
| `mcp_n8n-mcp_get_node (standard)` | **Get config (DEFAULT)** | <10ms | 1-2K |
| `mcp_n8n-mcp_get_node (minimal)` | Quick metadata | <5ms | 200 |
| `mcp_n8n-mcp_get_node (full)` | Complex debugging | <100ms | 3-8K |
| `mcp_n8n-mcp_get_node (docs)` | Learn usage | Fast | Medium |
| `mcp_n8n-mcp_get_node (search_properties)` | Find specific field | Fast | Small |
| `mcp_n8n-mcp_get_node (versions)` | Check versions | Fast | Small |

**Best Practice**: search → mcp_n8n-mcp_get_node(standard) → validate
...
**Conversion**: mcp_n8n-mcp_search_nodes returns BOTH formats:
```javascript
{
  "nodeType": "nodes-base.slack",          // Use with mcp_n8n-mcp_get_node, mcp_n8n-mcp_validate_node
  "workflowNodeType": "n8n-nodes-base.slack"  // Use with mcp_n8n-mcp_n8n_create_workflow
}
```
...
```javascript
// Step 1: Search
mcp_n8n-mcp_search_nodes({query: "http request"})
...
mcp_n8n-mcp_search_nodes({query: "ai agent", source: "all"})

// Get AI Agent documentation
mcp_n8n-mcp_get_node({nodeType: "nodes-langchain.agent", mode: "docs"})
