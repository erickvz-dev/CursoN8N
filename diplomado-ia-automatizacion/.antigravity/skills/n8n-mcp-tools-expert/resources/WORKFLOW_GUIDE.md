...
## mcp_n8n-mcp_n8n_update_partial_workflow (MOST USED!)
...
## mcp_n8n-mcp_n8n_deploy_template (QUICK START!)
...
const result = mcp_n8n-mcp_n8n_deploy_template({
  templateId: 2947,
...
## mcp_n8n-mcp_n8n_workflow_versions (VERSION CONTROL)
...
## mcp_n8n-mcp_n8n_test_workflow (TRIGGER EXECUTION)
...
// Just structure
mcp_n8n-mcp_n8n_get_workflow({id: "workflow-id", mode: "structure"})

// Minimal metadata
mcp_n8n-mcp_n8n_get_workflow({id: "workflow-id", mode: "minimal"})
```

---

## mcp_n8n-mcp_n8n_executions (EXECUTION MANAGEMENT)
...
3. EDIT (iterative! 56s avg between edits)
   mcp_n8n-mcp_n8n_update_partial_workflow({id, intent: "...", operations: [...]})
   → Make changes
...
6. MONITOR
   mcp_n8n-mcp_n8n_executions({action: "list", workflowId: id})
   mcp_n8n-mcp_n8n_executions({action: "get", id: execution_id})
```
...
**Most Important**:
1. **mcp_n8n-mcp_n8n_update_partial_workflow** is most-used tool (38,287 uses)
...
8. Use **mcp_n8n-mcp_n8n_deploy_template** for quick starts

**New Tools**:
- `mcp_n8n-mcp_n8n_deploy_template` - Deploy templates directly
- `mcp_n8n-mcp_n8n_workflow_versions` - Version control & rollback
- `mcp_n8n-mcp_n8n_test_workflow` - Trigger execution
- `mcp_n8n-mcp_n8n_executions` - Manage executions
