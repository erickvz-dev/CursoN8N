...
1. Configure node
   ↓
2. mcp_n8n-mcp_validate_node (23 seconds thinking about errors)
   ↓
3. Read error messages carefully
   ↓
4. Fix errors
   ↓
5. mcp_n8n-mcp_validate_node again (58 seconds fixing)
   ↓
...
const result3 = mcp_n8n-mcp_validate_node({
  nodeType: "nodes-base.slack",
  config,
  profile: "runtime"
});
...
**How to fix**:
1. Use `mcp_n8n-mcp_get_node` to see required fields
...
2. Use `mcp_n8n-mcp_get_node` to see options
...
**Solution**: Use `cleanStaleConnections` operation in `mcp_n8n-mcp_n8n_update_partial_workflow`
...
### mcp_n8n-mcp_n8n_validate_workflow (Structure)
...
**Steps**:
1. Note required fields from `mcp_n8n-mcp_get_node`
...
- Use `mcp_n8n-mcp_get_node` when unclear about requirements
...
- **[ERROR_CATALOG.md](resources/ERROR_CATALOG.md)** - Complete list of error types with examples
- **[FALSE_POSITIVES.md](resources/FALSE_POSITIVES.md)** - When warnings are acceptable
