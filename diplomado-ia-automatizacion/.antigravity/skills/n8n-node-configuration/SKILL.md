...
description: Operation-aware node configuration guidance. Use when configuring nodes, understanding property dependencies, determining required fields, choosing between mcp_n8n-mcp_get_node detail levels, or learning common configuration patterns by node type.
---
...
- `mcp_n8n-mcp_get_node` with `detail: "standard"` is the most used discovery pattern
...
1. **mcp_n8n-mcp_get_node({detail: "standard"})** - DEFAULT
   - Quick overview (~1-2K tokens)
   - Required fields + common options
   - **Use first** - covers 95% of needs

2. **mcp_n8n-mcp_get_node({mode: "search_properties", propertyQuery: "..."})** (for finding specific fields)
   - Find properties by name
   - Use when looking for auth, body, headers, etc.

3. **mcp_n8n-mcp_get_node({detail: "full"})** (complete schema)
...
1. Identify node type and operation
   ↓
2. Use mcp_n8n-mcp_get_node (standard detail is default)
   ↓
3. Configure required fields
   ↓
4. Validate configuration
   ↓
5. If field unclear → mcp_n8n-mcp_get_node({mode: "search_properties"})
   ↓
...
```javascript
const info = mcp_n8n-mcp_get_node({
...
## mcp_n8n-mcp_get_node Detail Levels
...
├─────────────────────────────────┤
│ YES → mcp_n8n-mcp_get_node (standard)       │
└─────────────────────────────────┘
         ↓
┌─────────────────────────────────┐
...
├─────────────────────────────────┤
│ YES → mcp_n8n-mcp_get_node({detail: "full"})│
└─────────────────────────────────┘
```
...
**Use mcp_n8n-mcp_get_node with search_properties mode**:
```javascript
mcp_n8n-mcp_get_node({
  nodeType: "nodes-base.httpRequest",
  mode: "search_properties",
...
1. Choose resource
2. Choose operation
3. Use mcp_n8n-mcp_get_node to see operation-specific requirements
...
const config = {...};
mcp_n8n-mcp_n8n_update_partial_workflow({...});  // YOLO
```
...
const config = {...};
const result = mcp_n8n-mcp_validate_node({...});
if (result.valid) {
...
1. **Start with mcp_n8n-mcp_get_node (standard detail)**
...
3. **Use search_properties mode when stuck**
   ...
   - `mcp_n8n-mcp_get_node({mode: "search_properties", propertyQuery: "..."})`

4. **Respect operation context**
   - Different operations = different requirements
   - Always check mcp_n8n-mcp_get_node when changing operation
...
- **[DEPENDENCIES.md](resources/DEPENDENCIES.md)** - Deep dive into property dependencies and displayOptions
- **[OPERATION_PATTERNS.md](resources/OPERATION_PATTERNS.md)** - Common configuration patterns by node type
...
1. Start with `mcp_n8n-mcp_get_node` (standard detail is default)
