...
### Using mcp_n8n-mcp_get_node with search_properties Mode
...
### Using mcp_n8n-mcp_get_node with Full Detail
...
**❌ Don't use when**:
- Simple configuration (use `mcp_n8n-mcp_get_node` with standard detail)
...
1. **Check dependencies when stuck**
   ```javascript
   mcp_n8n-mcp_get_node({nodeType: "...", mode: "search_properties", propertyQuery: "..."});
   ```
...
**Tools**:
- `mcp_n8n-mcp_get_node({mode: "search_properties"})` - Find property dependencies
- `mcp_n8n-mcp_get_node({detail: "full"})` - See complete schema with displayOptions
- `mcp_n8n-mcp_get_node` - See operation requirements (standard detail)
